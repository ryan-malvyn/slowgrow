...
const PostObject = ({ postData }: PostObjectProps) => {
    ...
    return (
        <article className="mb-10 p-4 pr-0 md:p-0">
            <p
                className={`${labelColor} px-4 py-0 rounded-full text-sm w-min text-nowrap mb-2`}
            >
                {labelContents}
            </p>
            <section className="flex pl-0.5 flex-col">
                <Link href={`/${slug.current}`}>
                    <h2 className="font-serif font-semibold mb-1 text-2xl underline underline-offset-4">
                        {title}
                    </h2>
                    <p className="font-light text-slate-400 mb-2 text-left">
                        {description}
                    </p>
